$(document).ready(function() {
	...
	//ID 중복검사 부분<input type=button value=ID중복검사 onClick="idcheck()">
	$('input[value=ID중복검사]').click(function() {

		if ($('#id').val().trim() == "") {//아이디 입력을 안했을때
			alert("ID를 입력하세요");
			$('#id').focus();
			return false;
		} else { //아이디를 입력했을때
			...
			if (pattern.test($('#id').val())) { //id 값이 정규 표현식을 만족하는지 체크합니다
	...
	$('#sel').on('change', function () {
		...
		if ($(this).val() == "") {//직접입력 선택된 경우
			...
			$('#domain').attr('readonly',true); //수정불가
			$('#domain').val($('#sel').val());
		}
	})//onchange sel
